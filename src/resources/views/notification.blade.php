...
@section('content')
<x-fe-portlet id="fe_notification" class="h-100p m-b-0 fullHeight" MailTargt="{{route('LF_Notifications')}}">
                    ...
                    <section>
                        <div class="mailbox-page clearfix">
                            ...
                            <div class="append-icon">
                                <input type="text" class="form-control form-white pull-right" id="email-search"
                                    placeholder="Search...">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade in active" id="recent">
                                <div class="messages-list withScroll show-scroll" data-padding="180"
                                    data-height="window">
                                    @foreach ($mails as $mail)
                                    <div class="message-item media" mail_id="{{$mail->id}}">
                                        <div class="media">
                                            {{-- <img src="{{asset('/feiron/felaraframe/images/shopping/mail.png')}}"
                                            alt="Mail Icon" width="40" class="sender-img"> --}}
                                            <i class="icon icon-envelope{{$mail->status=='read'?'-open':''}} f-40 sender-img"
                                                alt="Mail Icon"></i>
                                            <div class="media-body">
                                                <div class="sender">{{$mail->Sender->name??'From System'}}</div>
                                                <div class="subject">{!!(($mail->sender<=0)?'<span
                                                        class="label label-primary">
                                                        System</span>':'').(empty($mail->remarks)?"":'<span
                                                            class="label label-info">'.$mail->remarks.'</span>') !!}
                                                        <span class="subject-text">{{$mail->subject}}</span></div>
                                                <div class="date">
                                                    <strong>{{$mail->created_at->format('(D)M-d Y')}}</strong>
                                                </div>
                    ...
                    <section>
                        <div class="email-subject row">
                            ...
                            <div class="clearfix col-sm-12">
                                <p>from &nbsp;&nbsp;<strong><span class="sender badge badge-primary"></span></strong>
                                    &bull;&nbsp; <span class="date"></span></p>
                                <div class="pos-rel pull-left">
                                    <button type="button"
                                        class="btn btn-danger btn-rounded btn-sm m-l-10 btnRemoveNotification"
                                        tartget="">Delete</button>
                                </div>
                            </div>
                        </div>
